    ...
    for offset in numpy.arange(0, query.get_count(table), 50000):
        ...
        if len(results) > 0:
            with tqdm(total=(len(results)),
                      desc=f"{Fore.CYAN}[{table}]{Fore.RESET}::{Fore.YELLOW}Anonymizing DATA{Fore.RESET}{[i for i in keys[1:]]}{Fore.GREEN}") as pbar:
                for result in results:
                    list_of_fake_data = []
                    for em, val in enumerate(values[1:]):
                        list_of_fake_data.append(
                            fake_map[val](len(result[em + 1].split())) if result[em + 1] is not None else None)
                    fields_ = (
                        "{}={}".format(first,
                                       "'{}'".format(second.replace("'", "\''")) if second is not None else "Null")
                        for first, second in zip(keys[1:], list_of_fake_data))
                    query.update_query(f"UPDATE {table} SET {', '.join(fields_)} WHERE {keys[0]}={result[0]}")
                    pbar.update(1)
        else:
            break
...
def anonymize_json_basic(table, keys, values):
    ...
    for offset in numpy.arange(0, query.get_count(table), 50000):
        ...
        if len(results) > 0:
            with tqdm(total=(len(results)),
                      desc=f"{Fore.CYAN}[{table}]{Fore.RESET}::{Fore.YELLOW}Anonymizing JSON{Fore.RESET}{[i for i in keys[1:]]}{Fore.GREEN}") as pbar:
                for result in results:
                    json_template = values[1]
                    for fake in list_of_fake_keys:
                        while fake in json_template:
                            json_template = str(json_template).replace(f'{fake}', fake_map[fake](None), 1)
                    query.update_query(
